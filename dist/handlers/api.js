...
export async function handleHistoryRequest(request, env) {
    ...
    try {
      ...
      query {
        ...
        const appNames = await Promise.all(loginEvents.map(async (event) => {
...
async function fetchIdpDetails(accountId, idpId, env) {
    ...
    try {
        ...
        let response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/access/identity_providers/${idpId}`, {
            headers: {
                'Authorization': `Bearer ${env.BEARER_TOKEN}`,
                'Content-Type': 'application/json'
            }
        ...
        response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/access/identity_providers`, {
            headers: {
                'Authorization': `Bearer ${env.BEARER_TOKEN}`,
                'Content-Type': 'application/json'
            }
