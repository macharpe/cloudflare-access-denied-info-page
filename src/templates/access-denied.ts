...
@keyframes spin{
  ...
  100%{transform:rotate(360deg)}
    ...
    const jsContent = [
      ...
      "    '<button class=\"close-btn\" onclick=\"closeModal(event, this.closest(\\'.modal\\'))\">×</button>',",
      ...
      "    await displayDeviceInfo(userData);",
      "    await displayWarpInfo(userData, envData);",
      "    await displayNetworkInfo(networkData, envData);",
      ...
      "    const loadingEl = document.getElementById('loading');",
      "    if (loadingEl) {",
      "      loadingEl.innerHTML = '<div style=\"color: #dc2626; text-align: center; padding: 24px;\"><h3 style=\"font-size: 18px; font-weight: 600; margin-bottom: 8px;\">Loading Error</h3><p style=\"color: #6b7280; font-size: 14px;\">Failed to load information.</p></div>';",
      "    }",
      "  }",
      "}",
      "",
      ...
      "}",
      "",
      "async function displayDeviceInfo(userData) {",
      "  if (typeof document === 'undefined') return;",
      "  const deviceInfoEl = document.getElementById('device-info');",
      "  if (!deviceInfoEl) return;",
      "  const device = userData?.device?.result;",
      "  const identity = userData?.identity;",
      "  const warpMode = userData?.warpMode;",
      "  const osDisplayName = detectCurrentOS(userData);",
      "  ",
      "  // Try to infer device model and version from user-agent when not available from device data",
      "  let deviceModel = device?.model || 'Not available';",
      "  let osVersion = device?.os_version || 'Not available';",
      "  ",
      "  // Try to get real OS version from Client Hints API first (bypasses frozen User-Agent)",
      "  const realOSVersion = await getRealOSVersion();",
      "  if (realOSVersion) {",
      "    osVersion = realOSVersion;",
      "  }",
      "  ",
      "  // Use user-agent inference when in DoH mode (DNS-over-HTTPS) since device data is typically not available",
      "  const isDoHMode = warpMode?.mode === 'Gateway with DoH';",
      "  ",
      "  if (typeof navigator !== 'undefined' && navigator.userAgent && (deviceModel === 'Not available' || osVersion === 'Not available' || isDoHMode)) {",
      "    const userAgent = navigator.userAgent;",
      "    ",
      "    // Infer device model if not available or in DoH mode",
      "    if (deviceModel === 'Not available' || isDoHMode) {",
      ...
      "    }",
      "    ",
      "    // Infer OS version from user-agent if not available or in DoH mode",
      "    if (osVersion === 'Not available' || isDoHMode) {",
      "      if (userAgent.includes('iPhone') || userAgent.includes('iPad')) {",
      "      // Look for iPhone OS or OS in the user agent string",
      "      // Format: \"iPhone OS 18_7\" or \"OS 18_7\"",
      "      const iosMatch = userAgent.match(/(?:iPhone OS |OS )(\\d+[_\\d]*)/);",
      "      if (iosMatch && iosMatch[1]) {",
      "        // Convert underscores to dots for version format",
      ...
      "        if (ntVersion === '10.0') {",
      "          osVersion = userAgent.includes('Windows NT 10.0; Win64; x64') ? '10/11' : '10';",
      ...
      "  deviceHtml += `<div><span class=\"label\">Device ID:</span> <span class=\"value\" style=\"text-align: right; display: inline-block; width: auto; float: right;\">${identity?.device_id || 'Not available'}</span></div>`;",
      "  deviceHtml += `<div><span class=\"label\">Model:</span> <span class=\"value\">${deviceModel}</span></div>`;",
      ...
      "  deviceHtml += `<div><span class=\"label\">Version:</span> <span class=\"value\">${osVersion}</span></div>`;",
      "  deviceHtml += '</div>';",
      "  ",
      "  deviceInfoEl.innerHTML = deviceHtml;",
      "}",
      "",
      "async function fetchRealWarpStatus(envData) {",
      "  // Fetch trace endpoint directly from client to get accurate connection data (browser-to-Cloudflare)",
      "  try {",
      "    const orgName = envData?.ORGANIZATION_NAME || 'macharpe';",
      "    const response = await fetch(`https://${orgName}.cloudflareaccess.com/cdn-cgi/trace`);",
      "    if (!response.ok) return { isWarp: false, isGateway: false, isRbi: false, ip: null, httpProtocol: null };",
      ...
      "    });",
      "    ",
      "    // Parse HTTP protocol (e.g., 'http/2' -> 'HTTP/2', 'http/3' -> 'HTTP/3')",
      "    let httpProtocol = null;",
      "    if (data.http) {",
      "      httpProtocol = data.http.toUpperCase().replace('/', '/');",
      "    }",
      "    ",
      "    return {",
      "      isWarp: data.warp === 'on',",
      "      isGateway: data.gateway === 'on',",
      "      isRbi: data.rbi === 'on',",
      "      ip: data.ip || null,",
      "      httpProtocol: httpProtocol",
      "    };",
      "  } catch (error) {",
      "    console.error('Failed to fetch trace endpoint:', error);",
      "    return { isWarp: false, isGateway: false, isRbi: false, ip: null, httpProtocol: null };",
      "  }",
      "}",
      "",
      "async function getRealOSVersion() {",
      "  // Get real OS version using Client Hints API (not frozen User-Agent)",
      ...
      "    }",
      "  } catch (error) {",
      "    // Client Hints not available, will use fallback",
      "  }",
      "  ",
      "  return null;",
      "}",
      "",
      "async function displayWarpInfo(userData, envData) {",
      "  if (typeof document === 'undefined') return;",
      "  const warpInfoEl = document.getElementById('warp-info');",
      "  if (!warpInfoEl) return;",
      "  ",
      "  // Fetch real WARP status from trace endpoint",
      "  const realStatus = await fetchRealWarpStatus(envData);",
      "  ",
      "  const identity = userData?.identity;",
      "  const warpMode = userData?.warpMode || {};",
      "  ",
      "  // Determine if Gateway is enabled - check both trace endpoint and device API mode",
      "  // DNS-only mode (DoH) means Gateway is enabled even if warp tunnel is off",
      "  let isGatewayEnabled = realStatus.isGateway;",
      "  ",
      "  // Check device API mode first",
      "  if (warpMode.mode === 'Gateway with DoH' || warpMode.serviceMode === 'doh') {",
      "    isGatewayEnabled = true; // Trust device API for DNS-only mode",
      "  }",
      "  ",
      "  // Fallback: If user is authenticated but no WARP tunnel and no device details,",
      "  // they might be on mobile DNS-only mode (common on iPhone)",
      "  if (!realStatus.isWarp && !realStatus.isGateway && !warpMode.serviceMode) {",
      "    const hasIdentity = identity && identity.user_email;",
      "    const isMobile = typeof navigator !== 'undefined' && navigator.userAgent && ",
      "      (navigator.userAgent.includes('iPhone') || navigator.userAgent.includes('iPad') || navigator.userAgent.includes('Android'));",
      "    ",
      "    if (hasIdentity && isMobile) {",
      "      // Likely DNS-only mode on mobile (device not enrolled but Gateway DNS filtering active)",
      "      isGatewayEnabled = true;",
      "    }",
      "  }",
      "  ",
      "  const warpStatus = realStatus.isWarp ? 'Connected' : 'Disconnected';",
      "  const gatewayStatus = isGatewayEnabled ? 'Enabled' : 'Disabled';",
      "  const rbiStatus = realStatus.isRbi ? 'Enabled' : 'Disabled';",
      "  ",
      "  let warpHtml = '<h3>🌐 Connection Status</h3>';",
      "  warpHtml += '<div class=\"field\">';",
      "  ",
      "  // Display WARP/Gateway/RBI status first",
      "  warpHtml += `<div><span class=\"label\">WARP Status:</span> <span class=\"status ${realStatus.isWarp ? '' : 'disconnected'}\">${warpStatus}</span></div>`;",
      "  warpHtml += `<div><span class=\"label\">Gateway Status:</span> <span class=\"status ${isGatewayEnabled ? '' : 'disconnected'}\">${gatewayStatus}</span></div>`;",
      "  warpHtml += `<div><span class=\"label\">Remote Browser Isolation:</span> <span class=\"status ${realStatus.isRbi ? '' : 'disconnected'}\">${rbiStatus}</span></div>`;",
      "  ",
      "  // Display enhanced WARP mode information",
      "  let displayMode = warpMode.mode;",
      "  ",
      "  // If no mode is set, infer from realStatus and Gateway detection",
      "  if (!displayMode) {",
      "    if (isGatewayEnabled && realStatus.isWarp) {",
      "      displayMode = 'Gateway with WARP';",
      "    } else if (isGatewayEnabled && !realStatus.isWarp) {",
      "      displayMode = 'Gateway with DoH';",
      "    } else if (realStatus.isWarp && !isGatewayEnabled) {",
      "      displayMode = 'WARP (Consumer)';",
      "    } else {",
      "      displayMode = 'Unknown';",
      "    }",
      "  }",
      "  ",
      "  if (displayMode) {",
      "    warpHtml += `<div><span class=\"label\">Mode:</span> <span class=\"status ${realStatus.isWarp ? '' : 'disconnected'}\">${displayMode}</span></div>`;",
      "  }",
      "  ",
      "  // Display profile name if not default",
      "  if (warpMode.profileName && warpMode.profileName !== 'Default') {",
      "    warpHtml += `<div><span class=\"label\">Profile:</span> <span class=\"value\">${warpMode.profileName}</span></div>`;",
      "  }",
      "  ",
      "  // Display client version if available",
       "  if (warpMode.clientVersion) {",
       "    warpHtml += `<div><span class=\"label\">Client Version:</span> <span class=\"value\">${warpMode.clientVersion}</span></div>`;",
       "  }",
       "  ",
       "  // Display service mode if available (technical detail)",
       "  if (warpMode.serviceMode) {",
       "    warpHtml += `<div><span class=\"label\">Service Mode:</span> <span class=\"value\">${warpMode.serviceMode}</span></div>`;",
       "  }",
       "  ",
       "  // Display CF One Client virtual IP addresses if available",
       "  if (warpMode.virtualIpv4) {",
       "    warpHtml += `<div><span class=\"label\">CF One Client IPv4:</span> <span class=\"value\" style=\"font-family:monospace\">${warpMode.virtualIpv4}</span></div>`;",
       "  }",
       "  if (warpMode.virtualIpv6) {",
       "    warpHtml += `<div><span class=\"label\">CF One Client IPv6:</span> <span class=\"value\" style=\"font-family:monospace\">${warpMode.virtualIpv6}</span></div>`;",
       "  }",
       "  warpHtml += '</div>';",
      "  ",
      ...
      "}",
      "",
      "async function displayNetworkInfo(networkData, envData) {",
      "  if (typeof document === 'undefined') return;",
      ...
      "  }",
      "  ",
      "  // Fetch real connection details from trace endpoint (browser-to-Cloudflare)",
      "  const realConnectionData = await fetchRealWarpStatus(envData);",
      "  ",
      "  // Determine HTTP protocol display based on WARP mode",
      "  let httpProtocol = realConnectionData.httpProtocol || networkData.httpProtocol || 'Unknown';",
      "  ",
      "  // When WARP tunnel is active (warp=on), the actual connection uses HTTP/3 MASQUE tunnel",
      "  // even though browser connects to WARP proxy via HTTP/2 locally",
      "  if (realConnectionData.isWarp) {",
      "    // WARP tunnel (full or consumer mode) always uses HTTP/3",
      "    httpProtocol = 'HTTP/3';",
      "  }",
      "  // Otherwise use trace endpoint's http value as-is (covers DNS-only, no WARP, direct connections)",
      "  ",
      "  let networkHtml = '<h3>🌐 Network Information</h3>';",
      "  networkHtml += '<div class=\"field\">';",
      "  networkHtml += `<div><span class=\"label\">Public IP:</span> <span class=\"value\">${realConnectionData.ip || networkData.ip || 'Unknown'}</span></div>`;",
      "  networkHtml += `<div><span class=\"label\">HTTP Protocol:</span> <span class=\"value\">${httpProtocol}</span></div>`;",
      "  networkHtml += `<div><span class=\"label\">Country:</span> <span class=\"value\">${networkData.country || 'Unknown'}</span></div>`;",
      "  networkHtml += `<div><span class=\"label\">Region:</span> <span class=\"value\">${networkData.region || 'Unknown'}</span></div>`;",
      "  networkHtml += `<div><span class=\"label\">Location:</span> <span class=\"value\">${networkData.city || 'Unknown'}</span></div>`;",
      "  networkHtml += `<div><span class=\"label\">Edge Location:</span> <span class=\"value\">${networkData.edgeLocation || 'Unknown'}</span></div>`;",
      "  networkHtml += `<div><span class=\"label\">Timezone:</span> <span class=\"value\">${networkData.timezone || 'Unknown'}</span></div>`;",
      ...
      "  const isDoHMode = warpMode?.mode === 'Gateway with DoH';",
      "  ",
      "  // If in DoH mode, prioritize user-agent since device data is typically not available",
      "  if (isDoHMode && typeof navigator !== 'undefined' && navigator && navigator.userAgent) {",
      "    const userAgent = navigator.userAgent;",
      "    if (userAgent.includes('iPhone') || userAgent.includes('iPad')) {",
      ...
      "  }",
      "  ",
      "  // Otherwise try to determine OS from device type (most reliable for WARP enrolled devices)",
      "  if (warpMode?.deviceType) {",
      "    const deviceType = warpMode.deviceType.toLowerCase();",
      "    ",
      "    // Common device type mappings from Cloudflare Zero Trust",
      "    if (deviceType.includes('windows') || deviceType === 'desktop') {",
      "      return 'Windows';",
      "    } else if (deviceType.includes('mac') || deviceType === 'macos') {",
      "      return 'macOS';",
      "    } else if (deviceType.includes('linux')) {",
      "      return 'Linux';",
      "    } else if (deviceType.includes('ios') || deviceType === 'mobile_ios') {",
      "      return 'iOS';",
      "    } else if (deviceType.includes('android') || deviceType === 'mobile_android') {",
      "      return 'Android';",
      "    }",
      "  }",
      "  ",
      "  // Second try to determine OS from user-agent (fallback when device type not available)",
      ...
      "  }",
      "  ",
      "  // Fallback to device information if neither device type nor user-agent are conclusive",
      ...
      "  ",
      "  // Try OS field first if it looks reliable",
      "  if (osField && !osField.toLowerCase().includes('vmware') && !osField.toLowerCase().includes('virtualbox')) {",
      "    if (osField.toLowerCase().includes('windows')) return 'Windows';",
      "    if (osField.toLowerCase().includes('mac') || osField.toLowerCase().includes('darwin')) return 'macOS';",
      "    if (osField.toLowerCase().includes('linux')) return 'Linux';",
      "    if (osField.toLowerCase().includes('ios')) return 'iOS';",
      "    if (osField.toLowerCase().includes('android')) return 'Android';",
      "    return osField;",
      "  }",
      "  ",
      "  // Version-based detection as final fallback",
      ...
      "  if (!event || !modal || typeof document === 'undefined') return;",
