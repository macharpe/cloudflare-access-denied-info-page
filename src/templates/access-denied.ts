...
@keyframes spin{
  ...
  100%{transform:rotate(360deg)}
    ...
    const jsContent = [
      ...
      "  ",
      "  // Display WARP/Gateway status first",
      "  warpHtml += `<div><span class=\"label\">WARP Status:</span> <span class=\"status ${identity?.is_warp ? '' : 'disconnected'}\">${warpStatus}</span></div>`;",
      "  warpHtml += `<div><span class=\"label\">Gateway Status:</span> <span class=\"status ${identity?.is_gateway ? '' : 'disconnected'}\">${gatewayStatus}</span></div>`;",
      "  ",
      "  // Display enhanced WARP mode information",
      "  let displayMode = warpMode.mode;",
      "  ",
      "  // If no mode is set, try to infer DoH mode from gateway/warp status",
      "  if (!displayMode) {",
      "    // DNS-only mode: Gateway enabled but WARP not connected",
      "    if (identity?.is_gateway && !identity?.is_warp) {",
      "      displayMode = 'Gateway with DoH';",
      "    }",
      "    // Another common pattern: Gateway enabled with minimal WARP (DNS-only)",
      "    else if (identity?.is_gateway && identity?.is_warp && typeof navigator !== 'undefined' && navigator.userAgent) {",
      "      const userAgent = navigator.userAgent;",
      "      // Mobile devices often use DNS-only mode even when WARP shows as enabled",
      "      if (userAgent.includes('iPhone') || userAgent.includes('iPad') || userAgent.includes('Mobile')) {",
      "        displayMode = 'Gateway with DoH';",
      "      }",
      "    }",
      "    // Fallback: If Gateway is enabled on mobile device, assume DoH mode",
      "    else if (identity?.is_gateway && typeof navigator !== 'undefined' && navigator.userAgent) {",
      "      const userAgent = navigator.userAgent;",
      "      if (userAgent.includes('iPhone') || userAgent.includes('iPad') || userAgent.includes('Mobile')) {",
      "        displayMode = 'Gateway with DoH';",
      "      }",
      ...
      "  ",
      "  if (displayMode) {",
      "    warpHtml += `<div><span class=\"label\">Mode:</span> <span class=\"status ${identity?.is_warp ? '' : 'disconnected'}\">${displayMode}</span></div>`;",
      "  }",
      "  ",
      ...
      "  networkHtml += `<div><span class=\"label\">Public IP:</span> <span class=\"value\">${networkData.ip || 'Unknown'}</span></div>`;",
      "  networkHtml += `<div><span class=\"label\">HTTP Protocol:</span> <span class=\"value\">${networkData.httpProtocol || 'Unknown'}</span></div>`;",
